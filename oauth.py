...
SPREADSHEET_URL_TEMPLATE = 'https://docs.google.com/feeds/download/spreadsheets/Export?exportFormat=csv&key=%s'
...
#             get_document(config.COPY_GOOGLE_DOC_KEY, config.COPY_PATH)
...
def oauth_required(f):
    ...
    def decorated_function(*args, **kwargs):
        from flask import request
        credentials = get_credentials()
        if config.COPY_GOOGLE_DOC_KEY and (not credentials or not credentials.valid):
...
def get_document(key, file_path):
    ...
    response = config.authomatic.access(credentials, url)
    ...
    with open(file_path, 'wb') as writefile:
        writefile.write(response.content.encode('utf-8'))
